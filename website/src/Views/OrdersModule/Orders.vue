...
<template class="h-screen">
  <main>
    ...
    <body>
      <div v-for="(nerf, index) in orders" :key="nerf.id" class="mt-4 flex flex-col gap-4 flex-wrap flex-shrink">
        <ElemDetail class="p-6">
          <template #imgs>
            <div v-if="orderImgs.length">
              <img :src="orderImgs[0]" alt="Image" class="w-full h-full object-cover">
            </div>
          </template>

          <template #title>
            <h2 class="text-md font-semibold">Order n°{{ nerf.id.split("-").pop() }}</h2>
          </template>

          <template #details>
            <h3 class="text-sm font-medium mb-[-1%]">Nombre d'articles: {{ nerf.order_items.length }}</h3>
            <h3 class="text-sm font-medium mt-[-1%]">Commande passée le {{ nerf.created_at.split("T")[0] }}</h3> <!-- OU  order.created_at.substring(0, order.created_at.lastIndexOf("T")) -->
          </template>
        ...
        <Transition name="slide">
          <div v-if="showDetails" class="max-h-fit flex justify-center">
          <div class="border border-y-0 border-x-gray-500 w-[90%] flex flex-col justify-center mt-[-3%]">

          <div v-for="item in nerf.order_items"
          :key="item.item_id"
          class=" w-[100%]">
            <ElemDetail class="p-6">
              <template #imgs>
                <div v-if="item.material_items.image">
                  <img :src="item.material_items.image" alt="Image" class="w-full h-full object-cover">
                </div>
              </template>
              <template #title>
        ...
        <div class="flex justify-center">
          <hr v-if="index >= orders.length - 1" class="mt-[-1.8%] border-t border-gray-500 w-[90%]">
        </div>
      </div>
    </body>
  </main>
